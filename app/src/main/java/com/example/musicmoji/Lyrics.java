    ...
    private void getLyrics(String Artist, String Title, String Album) {
        ...
        try {
            ...
            try {
                ...
                lyrics = "";
                // Set this to "en" in case user is trying to play a server song in offline mode
                lyricsLanguageTag = "";
                e.printStackTrace();
            }
...
// Set variables, find early problems, set up translator
        ...
        // Moved it here so that we can identify language before doing anything if there is no language tag
        ...
        if (lyrics != null && !lyrics.equals("")) {
            ...
            // If there is no language tag, identify it using Watson API (first 100 characters should give enough accuracy)
            if (lyricsLanguageTag.equals("??") || lyricsLanguageTag.equals("")) {
                IdentifyOptions identifyOptions = new IdentifyOptions.Builder()
                        .text(lyrics.substring(0,100))
                        .build();

                IdentifiedLanguages languages = languageTranslator.identify(identifyOptions)
                        .execute().getResult();

                lyricsLanguageTag = languages.getLanguages().get(0).getLanguage();
            }
        }
        ...
        else if (context.getSharedPreferences("SPOTIFY", Context.MODE_PRIVATE).getString("token", "").equals("OFFLINE")) {
            new Handler(Looper.getMainLooper()).post(() -> {
                lyric_container.setText(R.string.Lyrics_offlineNotSaved);
            });
        }
...
//    If it's a non-English song and we are not translating it, we need to translate it word-by-word to English
                    ...
                    for (String lyric : lineList) {
                        ...
                        // "te" and "se" are Spanish indirect object pronouns
                        // Translated to tea and the Swedish flag, respectively
                        // "si" (no accent) means "if" but is translated to "yes"
                        if (originalLyric.equals("te") || originalLyric.equals("se") || originalLyric.equals("si")) {
                            continue;
                        }
...
// It's easier just to check for them individually than try to make a rule
                        ...
                        // Returns yawning emoji
                        // Not present in emoji library
                        if (lyric.startsWith("bore") || lyric.equals("boring") || lyric.startsWith("yawn")) {
                            lineLyrics.append("(\uD83E\uDD71) ");
                            canGenerateEmojis = true;
                            continue;
                        }
...
// 5. Add the processed line to emojiLyrics (translate if need be)
                    ...
                    if (!lyricsLanguageTag.equals(preferredLanguage) && (!preferredLanguage.equals("en"))) {
                        ...
                        punctuationReturned = punctuationReturned.replaceAll("('til)\\b|(til)\\b", "until").replaceAll("('Til)\\b|(Til)\\b", "Until");
